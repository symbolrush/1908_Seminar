...
Tools, welche das (automatisierte) Testen unterstützen gibt es seit Jahren viele. Alleine in \textsf{R} gibt es mit `svUnit` und `rUnit` seit über 10 Jahren zwei Implementierungen des XUnit Frameworks.  
Warum also braucht es mit `testthat` nochmals ein neues Framework?  

In Ihrem Buch "The Pragmatic Programmer" argumentieren Andrew Hunt und David Thomas, dass Testing primär ein kulturelles Problem sei und weniger ein technisches. [@Hunt2000] Das ist einsichtig. Aus technischer Sicht sind Unit Tests einfach Codefragmente, welchee den zu testenden Code aufrufen und die Rückgabewerte mit erwarteten Resultaten vergleichen. Solcher Code kann offensichtlich mit jeder Programmiersprache geschrieben werden. Im Prinzip ist hierzu auch kein formales Testing Framework notwendig. Hunt und Thomas schreiben denn auch weiter, dass die Kultur des Testens in jedem Projekt, unabhängig von der Programmiersprache, eingebunden werden kann.
...
In \textsf{R} existieren viele Testing Ansätze. Die relevante Literatur zum Thema unterscheidet sich nur geringfügig in den genannten Testing Ansätze. Durch Kombination der verschiedenen Artikel können 5 verschiedene Testing Ansätze eruiert werden.
...
1. das richtige Resultat einer Berechnung zurückgibt, ob
2. die Klasse des Rückgabeobjektes `numeric` ist und, ob
...
Drittens nennt er die Cacheing Funktion von `testthat` als Vorteil, da so die Wartezeiten verkürzt werden.
