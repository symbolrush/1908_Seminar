...
In diesem Stil gehören Methoden nicht zu Klassen, sondern zu generischen Funktionen. Die generische Funktion sucht sich beim Aufruf, die zu den Klassen der Attributen passende Methode[^12]. [@Chambers2014],[@Wickham2015a]
...
### `RUnit` und `svUnit`   
Mit `RUnit` und `svUnit` existieren zwei Packages, welche das XUnit Framework in $R$ implementieren. `RUnit` und `svUnit` sind ähnlich aufgebaut und orientieren sich an `jUnit`. Die Syntax beider Packages ist nahezu identisch zur `jUnit` Syntax. Das folgende Minimalbeispiel gibt einen Eindruck darüber, wie das Testing mit `RUnit` funktioniert. Auf ein `svUnit` Beispiel wird verzichtet.

Das folgende Beispiel wird im Folgenden auch in den Frameworks `testit` und `testthat` aufgebaut. Daher an dieser Stelle eine kurze Erläuterung. Getestet wird eine kleine Funktion `add()`, welche zwei Zahlen addiert. Die Funktion wird im Beispiel jeweils gezeigt. Drei Testfälle testen, ob die Funktion `add()` das richtige Resultat einer Berechnung zurückgibt, ob die Klasse des Rückgabeobjektes `numeric` ist und, ob die Funktion einen Fehler wirft, wenn nicht beide Zahlen als `numeric` übergeben werden.
...
Beide packages konnten sich in der $R$ Community nicht breit durchsetzen. Die einzige Ausnahme davon sind die Packages des Bioconductor Projektes[^25], welche `RUnit` relativ häufig eingesetzt haben, seit einigen Jahren jedoch mehr und mehr nach `testthat` switchen. Es gibt mit `runittotestthat`[^26] sogar ein R Package, welches `RUnit` Tests automatisch in `testthat` Tests umwandelt. 

Hadley Wickham und Yihui Xie argumentieren pauschal, dass der Einarbeitungs- und Einrichtungsaufwand wohl bei beiden Packages zu gross sei. [@Wickham2011] und [@YihuiXie] Dasselbe Argument nennt auch Bioconductor als eine der Schwächen des `RUnit` Frameworks.[^27] Richie Cotton nennt in seinem Blogbeitrag "Automatically convert RUnit tests to testthat tests", in welchem er sein Paket `runittotestthat` vorstellt noch drei weitere explizitere Gründe, warum er von `RUnit` zu `testthat` gewechselt hat. [@Cotton]

Erstens ist die Integration in den Workflow besser. Wo mit `RUnit` ein ganzes File von Hilfscode nötig war, um ein Paket zu testen, genügt es in `testthat` die Funktion `test_package()` aufzurufen. Zweitens gibt es in `testthat` elegantere Mechanismen, um Warnungen zu testen. Das folgende Beispiel aus [@Cotton] zeigt einen Test auf eine Warnung in `RUnit` und `testthat`. Das Beispiel testet, ob eine `sqrt()` Funktion eine Warnung wirft, bei einem negativen Input.
...
Ein weiterer wichtiger Nachteil der Pakete `RUnit` und `svUnit` ist zudem die Tatsache, dass beide Pakete seit einigen Jahren nicht mehr aktiv weiterentwickelt werden. 


[^25]: http://bioconductor.org/
[^26]: https://cran.r-project.org/web/packages/runittotestthat/index.html
[^27]: http://bioconductor.org/developers/how-to/unitTesting-guidelines/#choosingTestFramework


https://4dpiecharts.com/2014/05/12/automatically-convert-runit-tests-to-testthat-tests/
...
Das folgende einleitende Beispiel zeigt dieselben Tests der `add()` Funktion, welche schon mit `RUnit` und `testit` gezeigt wurden.
...
Die kurze Einleitung (Kapitel 1) zur Programmiersprache $R$ zeigt auf, was $R$ ist und in welchen Kontexten $R$ primär genutzt wird. Die Eigenheit der hohen Interaktivität ermöglicht schnelles Arbeiten in der Konsole und eben auch das im Zitat erwähnte interaktive Testen in der Konsole. Hier wird klar, warum diese Praxis sich überhaupt verbreiten konnte.

In Kapitel 2 wird erstens aufgezeigt, warum die Praxis der nicht gespeicherten interaktiven Tests auch im Bereich des Scientific Computing nicht eine gute Praxis ist. Zweitens, welche anderen Ansätze als `testthat` es gibt, um in $R$ automatisiert zu testen. Drittens wird augezeigt, dass die "Kultur des Testens" enorm wichtig ist. Hier wird klar, warum es `testthat` besser gelingt als anderen Ansätzen, die Praxis der interaktiven Tests in grossem Masse aufzubrechen.
