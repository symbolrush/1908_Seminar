...
abstract: |
  Dieser Seminarbeitrag stellt das `testthat` Package für automatisiertes Testen von R packages vor. Einleitend wird eine kurze Einführung in R gegeben und einige grundlegende Gedanken zum Testing allgemein und spezifisch zum Testing im Scientific Computing vorgestellt. Einige Gedanken zur Erweiterung von `testthat` zur Integration weiterer Konzepte des Testings in R runden den Hauptteil ab.
---
...
Die folgenden Erläuterungen zum `testthat` Package basieren im Wesentlichen auf den beiden wichtigsten Primärquellen des Autors Hadley Wickham selber. Dies ist ein Paper, welches Hadley Wickham beim Erscheinen von `testthat` geschrieben hat [@Wickham2011] und das Buch "R Packages", in welchem Hadley Wickham den Entwicklungsprozess von R Packages detailliert beschreibt und dabei auch auf das Testing mit `testthat` eingeht. [@Wickham2015]
...
[^31]: https://cran.r-project.org/web/packages/devtools/devtools.pdf
