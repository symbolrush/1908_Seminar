...
## `covr`  
Das R Package `covr`[^43][^44] stellt ein Werkzeug zur Verfügung um die Test Coverage zu messen. Mit `covr` kann festgestellt werden, welcher Anteil des Codes durch mindestens einen Unit Test durchlaufen werden und stellt die Resultate sowohl in KPIs als auch in interaktiven Dokumenten dar. Mit `covr` kann schnell festgestellt werden, welche Codezeilen durch keinen Testfall aufgerufen werden. `covr` ist relativ neu und in keinem Artikel oder Buch erwähnt.
...
`testthat` hat sich in den letzten Jahren als der de-facto Standard für das automatisierte Testen in R etabliert. Dieser Status ist gerechtfertigt. `testthat` bietet viele nützliche Funktionen für die Formulierung von Tests und ist gut in RStudio integriert. Die vielfältigen, natürlichsprachlich formulierten `expect_something` Funktionen erlauben es Tests auf einfache Art zu schreiben. Die Integration in den Workflow ist derart einfach, dass kaum Zusatzaufwand besteht. `testthat` ist das vollständigste Unit-Testing Framework in R und wenn die explosionsartige Verbreiterung von `testthat` fortschreitet ist das von Hadley Wickham als verbreitete Praxis beschriebene interaktive Testen in der Konsole bald Schnee von gestern.
